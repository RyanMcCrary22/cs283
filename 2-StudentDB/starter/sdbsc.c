...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    if( result == -1 ){
        // if result is -1 then seek failed to reach the offset
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    // set up the student to be written
    buff_student.id = id;
    buff_student.gpa = gpa;
    strcpy( &buff_student.fname, fname );
...
void print_student(student_t *s){
    // check for NULL Pointer or empty student 
    if( s->id == 0 || s == NULL ){
        printf(M_ERR_STD_PRINT);
    } 
    else {
        float calculated_gpa_s = (float) s->gpa / 100;
        printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
        printf(STUDENT_PRINT_FMT_STRING, s->id, s->fname, s->lname, calculated_gpa_s);
    }
}
